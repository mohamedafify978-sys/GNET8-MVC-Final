...
namespace GYMSystem.BLL.Services.classes
{
    public class MembershipService : IMembershipService
    {
        ...
        {
            var memberExists = await unitOfWork.GetRepository<Member>()
                .AnyAsync(m => m.Id == model.MemberId, ct);

            if (!memberExists)
                return Result.NotFound("Member not found.");

            var plan = await unitOfWork.GetRepository<Plan>()
                .GetByIdAsync(model.PlanId, ct);
            ...
            var hasActive = await unitOfWork.GetRepository<Membership>()
                .AnyAsync(m => m.MemberId == model.MemberId &&
            ...
            var entity = new Membership
            {
                ...
                EndDate = (model.StartDate ?? DateTime.Now)
                    .AddDays(plan.DurationDays)
            };

            unitOfWork.GetRepository<Membership>().Add(entity);

            var result = await unitOfWork.SaveChangesAsync(ct);

            return result > 0
                ? Result.Ok()
                : Result.Fail("Failed To Create New Membership");
        }
        ...
        public async Task<Result> DeleteActiveMembershipAsync(int memberId, CancellationToken ct = default)
        {
            ...
            return result > 0
                ? Result.Ok()
                : Result.Fail("Failed To Delete Membership");
        }
        ...
        public async Task<IEnumerable<MemberShipViewModel>> GetAllMembershipsAsync(CancellationToken ct = default)
        {
            var memberships = await unitOfWork.MembershipRepository
                .GetAllMembershipsWithMemberAndPlanAsync(
                    m => m.EndDate > DateTime.Now,
                    ct);
        ...
        {
            var plans = await unitOfWork.GetRepository<Plan>()
                .GetAllAsync(ct: ct);
